...
// Fixed download sources data with corrected percentages 
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        {/* Download Sources - Fixed percentage display */}
        <Card>
          <CardHeader>
            <CardTitle style={{ fontFamily: '"Roboto Slab", serif', fontWeight: 500 }}>
              Download Sources
            ...
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
                  ...
                  nameKey="name"
                  label={({ name, payload }) => {
                    // Use the pre-calculated percent directly from the data
                    const source = downloadSources.find(s => s.name === name);
                    return `${name}: ${source ? source.percent : 0}%`;
                  }}
                >
